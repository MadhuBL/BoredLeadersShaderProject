...
public class StompEffectTextAnimationView : MonoBehaviour
{
    ...
    void InitAnimeSetUp()
    {
        ...
        CommonVfxEffect.SetCustomMatPara(_flashMaterial,"_GhostColorBoost", _ghostBoostMax);
        CommonVfxEffect.SetCustomMatPara(_flashMaterial,"_GhostBlend", _ghostBlendMin);
        CommonVfxEffect.SetCustomMatPara(_flashMaterial,"_GhostTransparency", _ghostTransparencyMin);

        CommonVfxEffect.SetCustomMatPara(_textImgMaterial,"_Alpha", 0);
    ...
    public void AnimateIn()
    {
        ...
        tweenSeq.Join(DOVirtual.Float(0, 1, _textFadeInDuration, v => {CommonVfxEffect.SetCustomMatPara(_textImgMaterial, "_Alpha",v); } )).SetEase(Ease.Linear);
        ...
        tweenSeq.Join(DOVirtual.Float(0, 1, _ghostMatAnimDuraton, v => {CommonVfxEffect.SetCustomMatPara(_textImgMaterial, "_ShineLocation",v); } )).SetEase(Ease.Linear);
    ...
    void AnimateOut()
    {
        ...
        Sequence tweenSeq = DOTween.Sequence();
        tweenSeq.Join(DOVirtual.Float(_ghostBoostMax, _ghostBoostMin, _ghostMatAnimDuraton, v => {CommonVfxEffect.SetCustomMatPara(_flashMaterial,"_GhostColorBoost",v); } )).SetEase(Ease.Linear);
        tweenSeq.Join(DOVirtual.Float(_ghostBlendMin, _ghostBlendMax, _ghostMatAnimDuraton, v => {CommonVfxEffect.SetCustomMatPara(_flashMaterial,"_GhostBlend",v); } )).SetEase(Ease.Linear);
        tweenSeq.Join(DOVirtual.Float(_ghostTransparencyMin, _ghostTransparencyMax, _ghostMatAnimDuraton, v => {CommonVfxEffect.SetCustomMatPara(_flashMaterial,"_GhostTransparency",v); } )).SetEase(Ease.Linear);
        tweenSeq.Append(_flashImg.DOFade(0, _flashFadeOutDuration)).SetEase(Ease.Linear);
        tweenSeq.Insert(_ghostMatAnimDuraton - _textFadeOutDuration, DOVirtual.Float(1, 0, _textFadeOutDuration, v => {CommonVfxEffect.SetCustomMatPara(_textImgMaterial, "_Alpha",v); } )).SetEase(Ease.Linear);
